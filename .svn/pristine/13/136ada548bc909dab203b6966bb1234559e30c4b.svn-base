...
class YTController extends Controller
{
    ...
    public function refound(Request $request) {
        $input = $request->all();
        $client_ip = empty($this->getIp()) ? '127.0.0.1' : $this->getIp();
        \Log::info('refound-input', ['refound-input' => $input, 'ip' => $client_ip]);

        // 退款参数验证
        $transaction_no = isset($input['transaction_no']) ? $input['transaction_no'] : '';
        $sign = isset($input['sign']) ? trim($input['sign']) : '';
        $sign_result = $this->checkRefundSign($transaction_no, $sign);
        if (!$sign_result)
            return $this->api_helper->ajaxReturnError('签名失败');
        ...
        // 验证退款商户 ip 地址
        $merchant_info = $this->merchant->queryMerchantInfo($mid);
        if ($merchant_info) {
            $merchant_ips = array();
            foreach ($merchant_info as $sub_merchant_info) {
                array_push($merchant_ips, $sub_merchant_info['ip']);
            }

            if (!in_array($client_ip, $merchant_ips)) {
                return $this->api_helper->ajaxReturnError('非法 ip 地址');
            }
        } else {
            return $this->api_helper->ajaxReturnError('非法商户');
        }
        ...
        if ($result->rc == '00') {
            return $this->api_helper->ajaxReturnSuccess(array('transaction_no' => $transaction_no));
        } else {
     ...
     * @return bool
     */
    private function checkRefundSign($transaction_no,$sign) {
        if (empty($transaction_no))
            return false;
        $transaction_info = $this->transaction->transactionByTransactionNO($transaction_no);
        if (empty($transaction_info))
            return false;

        $merchant_id = isset($transaction_info['merchant_id']) ? $transaction_info['merchant_id'] : '';
        $merchant_info = $this->queryMerchantInfo($merchant_id);
        $merchant_security_key = isset($merchant_info['security_key']) ? $merchant_info['security_key'] : '';
        $sign_string_local = $transaction_no.$merchant_id.$merchant_security_key;
        \Log::info('checkRefundSign', ['merchant_security_key' => $merchant_security_key, 'sign' => $sign, 'sign_string_local' => md5($sign_string_local)]);
        if ($sign === md5($sign_string_local)) {
            return true;
        } else {
            return false;
